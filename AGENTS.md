...
Reusable Blade components in `resources/views/components/*.blade.php` (`<x-alert>`, `<x-badge>`, `<x-avatar>`, `<x-button>`, `<x-breadcrumb>`, `<x-card>`, `<x-page-header>`, `<x-icon>`, `<x-brand>`), each wrapping real Tabler markup/classes. **Read `docs/components/README.md` first** — it indexes a usage doc per component (props, variants, accessibility notes, Tabler docs link) in `docs/components/*.md`. Follow that pattern (wrap Tabler markup in a prop-driven component, document it, use it on the Starter Kit page) when adding new components.
